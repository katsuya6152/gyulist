...
`,
						...
						for (var p = 0; p < d.length; p++) {
							...
							for (var S = 0; S < b.length; S++)
								for (
									...
									if ((i[y].upsert(O, c.boost, (Ae, He) => Ae + He), !s[j])) {
	...
	async function fe(t, e) {
		...
		const n = await A(window.searchData);
		(t.data = n),
	...
	document.addEventListener(H, (t) => {
		(ie = !0), (D = !1);
		const e = H == "touchstart" ? t.targetTouches[0] : t;
...
`),
	...
	async function Te() {
		...
		const e = await A(window.navigationData);
	...
	function Ke(t, e, n) {
		...
		for (const i of r) t.appendChild(_e(e, i, n));
	}
