...
import { getBreedingTrendsUseCase } from "../../../../src/application/use-cases/kpi/getBreedingTrends";
import type { KpiRepository } from "../../../../src/domain/ports/kpi";
import type { TrendAnalysisResult } from "../../../../src/domain/types/kpi";
import type { UserId } from "../../../../src/shared/brand";
import { ok } from "../../../../src/shared/result";

describe("getBreedingTrendsUseCase", () => {
	let mockKpiRepo: {
	...
	it("should get breeding trends successfully with specific months", async () => {
		const userId = 1 as UserId;
		const fromMonth = "2023-01";
		const toMonth = "2023-12";

		const mockTrendResult: TrendAnalysisResult = {
			period: {
				startPeriod: { year: 2023, month: 1 },
				endPeriod: { year: 2023, month: 12 }
			},
			trends: [],
			insights: [],
			recommendations: []
		...
		mockKpiRepo.analyzeBreedingTrends.mockResolvedValue(ok(mockTrendResult));

		const useCase = getBreedingTrendsUseCase({
			kpiRepo: mockKpiRepo
		});
		...
		expect(mockKpiRepo.analyzeBreedingTrends).toHaveBeenCalledWith({
			ownerUserId: userId,
			startPeriod: { year: 2023, month: 1 },
			endPeriod: { year: 2023, month: 12 },
			minDataPoints: 3
	...
	it("should get breeding trends with default parameters", async () => {
		const userId = 1 as UserId;
	...
	it("should get breeding trends with custom months parameter", async () => {
		const userId = 1 as UserId;
		const months = 6;

		const mockTrendResult: TrendAnalysisResult = {
			period: {
				startPeriod: { year: 2023, month: 7 },
				endPeriod: { year: 2024, month: 1 }
			},
			trends: [],
			insights: [],
	...
	it("should handle invalid period format", async () => {
		const userId = 1 as UserId;
		const invalidMonth = "invalid-format";
		...
		const result = await useCase({
			ownerUserId: userId,
			fromMonth: invalidMonth
		});

		expect(result.ok).toBe(false);
		if (!result.ok) {
			expect(result.error.type).toBe("ValidationError");
			expect(result.error.message).toBe("Invalid period format");
		}
	});

	it("should handle invalid period range", async () => {
		const userId = 1 as UserId;
		const fromMonth = "2023-12";
		const toMonth = "2023-01"; // 開始月が終了月より後
		...
		expect(result.ok).toBe(false);
		if (!result.ok) {
			expect(result.error.type).toBe("PeriodError");
	...
	it("should handle repository error", async () => {
		...
		mockKpiRepo.analyzeBreedingTrends.mockRejectedValue(
		...
		if (!result.ok) {
			...
			expect(result.error.message).toBe("Failed to get breeding trends");
		}
	});
});
