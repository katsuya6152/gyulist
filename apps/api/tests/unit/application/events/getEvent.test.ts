...
import { getEventUseCase } from "../../../../src/application/use-cases/events/getEvent";
import type { EventRepository } from "../../../../src/domain/ports/events";
import type { Event } from "../../../../src/domain/types/events";
...
describe("getEventUseCase", () => {
	...
	it("should get event successfully", async () => {
		...
		mockEventRepo.findById.mockResolvedValue(ok(mockEvent));

		const useCase = getEventUseCase({
		...
		if (result.ok) {
			...
			expect(result.value.eventType).toBe("INSEMINATION");
		}
		expect(mockEventRepo.findById).toHaveBeenCalledWith(eventId, userId);
	});

	it("should return error when event not found", async () => {
		const eventId = 999 as EventId;
		const userId = 1 as UserId;

		mockEventRepo.findById.mockResolvedValue(ok(null));
	...
	it("should handle repository error", async () => {
		...
		mockEventRepo.findById.mockRejectedValue(new Error("Database error"));
