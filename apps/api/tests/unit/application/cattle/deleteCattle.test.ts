...
import { deleteCattleUseCase } from "../../../../src/application/use-cases/cattle/deleteCattle";
...
describe("deleteCattleUseCase", () => {
	...
	it("should delete cattle successfully", async () => {
		...
		mockCattleRepo.delete.mockResolvedValue(ok(undefined));

		const useCase = deleteCattleUseCase({
		...
		if (result.ok) {
			expect(result.value.message).toBe("牛が正常に削除されました");
		}
		expect(mockCattleRepo.findById).toHaveBeenCalledWith(cattleId);
		expect(mockCattleRepo.delete).toHaveBeenCalledWith(cattleId);
	});
	...
	it("should return error when user lacks permission", async () => {
		...
		if (!result.ok) {
			...
			expect(result.error.message).toBe("この牛の削除権限がありません");
		}
	});
	...
	it("should handle repository error", async () => {
		...
		if (!result.ok) {
			...
			expect(result.error.message).toBe("Failed to delete cattle");
		}
	});
});
